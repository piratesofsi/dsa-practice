...
Handles insertion at the head, middle, and end.
...
Removes the last node by reaching the second-last node.
...
* If head has the value → move head forward
* Otherwise, search for the value
* Link previous node to next node to remove the matched node
* If value not found → no changes

Useful when position isn't known and only the value matters.
...
📌 **[Code](https://github.com/piratesofsi/dsa-practice/blob/main/Linked%20List/Linkedlist%20easy/implementation.java)**
Searches each node sequentially for a value.
Linear search → **O(n)**.
